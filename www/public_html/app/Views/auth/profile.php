...
<?= $this->section('main') ?>
  ...
  <div class="card p-3 my-3">
    <form action="<?= site_url('update-profile'); ?>" method="POST" accept-charset="UTF-8" onsubmit="updateProfile.disabled = true; return true;">
      <?= csrf_field() ?>
      <h6 class="pb-2 mb-0 mt-4">Nombres</h6>
      ...
      <h6 class="pb-2 mb-0 mt-4">Informacion de Contacto</h6>
      ...
      <div class="form-group row">
        <div class="col-sm-10">
          <button name="updateProfile" type="submit" class="btn btn-primary">Actualizar Perfil</button>
        </div>
      </div>
    </form>
  </div>
  ...
  <div class="card p-3 my-3">
    <form action="<?= site_url('change-password'); ?>" method="POST" accept-charset="UTF-8" onsubmit="changePassword.disabled = true; return true;">
      <?= csrf_field() ?>
      <h6 class="pb-2 mb-0 mt-4">Cambiar contraseña</h6>

      <div class="form-group row mt-3">
        <label class="col-sm-2 col-form-label">Contraseña anterior</label>
        <div class="col-sm-10">
      ...
      <div class="form-group row">
        <div class="col-sm-10">
          <button name="changePassword" type="submit" class="btn btn-primary">Actualizar contraseña</button>
        </div>
      </div>
    </form>
  </div>
  ...
  <div class="card p-3">
    <form action="<?= site_url('delete-account') ?>" method="POST" accept-charset="UTF-8" onsubmit="deleteAccount.disabled = true; return true;">
      <?= csrf_field() ?>
      <h6 class="pb-2 mb-0 mt-4">Remover Cuenta</h6>
      <p><?= lang('Eliminar informacion de la cuenta') ?></p>

      <div class="form-group row mt-3">
        <label class="col-sm-2 col-form-label">Password anterior</label>
        <div class="col-sm-10">
          <input type="password" name="password" class="form-control col-md-6" value="" minlength="5" required>
        </div>
      </div>
      ...
      <div class="form-group row">
        <div class="col-sm-10">
          <button name="deleteAccount" type="submit" class="btn btn-danger" onclick="return confirm('<?= lang('Está seguro que desea eliminar password') ?>')"> <?= lang('Eliminar cuenta') ?></button>
        </div>
      </div>
    </form>
  </div>
