...
class EvaluacionController extends Controller
{
		   ...
		   $sql.=" ON a.id_competencia=b.id_competencia INNER JOIN `eval_capacidades` c  ON c.id_capacidad=a.id_capacidad WHERE a.id_evaluacion=".$id_evaluacion." AND a.`active`=1 ORDER BY a.`orden`";
			...
			$dat_grup_preg=[];
                ...
                {	
					...
					{
					 $html_cab1.="<th colspan='".@$col_cap[$id_comp]."' >".$data_competencia[$id_comp]."</th>";
	...
	public function get_alumnos_examen()
	{
		...
		if($this->request->isAjax() )
		{
            $db = \Config\Database::connect();
			$id_evaluacion = $this->request->getPost('id_evaluacion');
			$idcolegio_sec = $this->request->getPost('idcolegio_sec');
			$get_id_alumno = $this->request->getPost('id_alumno');
			...
			if($numero_preguntas>0)
			{
				...
				{ 
				 $pregunta_p[$row->id_pregunta]=$row->pregunta;
				 $preg_correcta[$row->id_pregunta]=$row->correcta;
				 $preg_puntaje[$row->id_pregunta]=$row->puntaje;
				...
				if($get_id_alumno!='0' &&  $get_id_alumno!='')
				{
					...
					$filter1=" AND nombres like '%{$nom_ape[0]}%' AND apellidos like '%{$nom_ape[1]}%'  ";
				...
				( SELECT id_alumno,id_colegiosec,nombres,apellidos,dni,sexo FROM alumnos WHERE active=1 AND id_colegiosec={$idcolegio_sec} {$filter1} ) AS d
				LEFT JOIN 
				( SELECT b.id_detalle,a.id_examen,a.id_alumno,a.id_evaluacion,b.id_pregunta,b.opcion_alias,a.nota
				FROM `examen_alumnos` a LEFT JOIN  `examen_alumnos_det` b ON a.id_examen=b.id_examen WHERE a.id_evaluacion={$id_evaluacion} ORDER BY  b.id_pregunta) AS c
				ON d.id_alumno=c.id_alumno ORDER BY d.apellidos ";
				...
				{	
					foreach($result_data2 as $row)
					{
						...
						$lista_alumnos_sec[]=$row->nombres.",".$row->apellidos;
						$nota_alumno[$row->id_alumno]=$row->nota;
						$data_pr=(is_null($row->id_pregunta)?"0":$row->id_pregunta);
						$puntos_preg=(is_null($row->id_pregunta)?0:$preg_puntaje[$row->id_pregunta]);
						$puntos_c=0;
						if($puntos_preg>0)
				...
				//
					...
					{		
						...
						foreach($tipos_preguntas as $tipo_pre=>$val)
						{
							...
							if($val==2){ $add_class="col-5"; $cls1="alert alert-warning";}
				   ...
				   $('.ins2_opcionpreg').change(function(e)
					{
						...
						console.log('id_examen:'+idexamen);
						let nota=suma_notas_alumno(id_btn,idexamen);
						console.log('sumaaa:'+nota);
						...
						$.ajax({
							...
							if(datos!='0')
							{
								...
								$('#'+id_btn).addClass('up_opcionpreg');
								$('#cmb_seccion').trigger('change');
								/*$('select[id^='+id_comun+']').each(function(){
									if(id_btn!=$(this).attr('id'))
									{
										let id_cc=$(this).attr('id');
										if(!$('#'+id_cc).hasClass('up_opcionpreg'))
										{
											$(this).attr('data-idexamen',id_exa[1]);
											$(this).removeClass('ins2_opcionpreg');
											$(this).addClass('ins1_opcionpreg');
										}									
	...
	public  function tipo_pregunta($tipo){
		...
		$opciones[2]=['Correcta'=>'Correc','Parcial'=>'Parc','Incorrecta'=>'Incor']; // matematica ,nivel1
	...
	public  function tipo_pregunta_js($tipo){
		...
		$opciones[2]=['0'=>'0','Correcta'=>'Correc','Parcial'=>'Parc','Incorrecta'=>'Incor']; // matematica ,nivel1
		return $opciones[$tipo];
	}

	public function preguntas_puntaje($valor)
	{
		$puntajes=['Correcta'=>2,'Parcial'=>1,'Incorrecta'=>0]; 
